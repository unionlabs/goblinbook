// https://github.com/unionlabs/union/blob/d9ebbbdb10fff129fe2e557534fc7ff345ef1a1b/galoisd/pkg/lightclient/common.go#L1
...
// ANCHOR_END: verify-signature	
	...
	// ANCHOR_END: for-each-val-signature
		
		bitmapMask := lc.input.NbOfVal
		for i, signed := range bitmap {
			// ANCHOR: for-each-val-hash
			...
			leaf := h.Sum()
			// ANCHOR_END: for-each-val-hash

			// ANCHOR: for-each-val-reconstruct-pubkey
			...
			*/
			...
			// ANCHOR_END: for-each-val-reconstruct-pubkey

			// ANCHOR: for-each-val-bitmask
			cannotSign := lc.api.IsZero(bitmapMask)
			// ANCHOR_END: for-each-val-bitmask
	...
	// ANCHOR_END: for-each-val-collect

	// ANCHOR: final-constraints
